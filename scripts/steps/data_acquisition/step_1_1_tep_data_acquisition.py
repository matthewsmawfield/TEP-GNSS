    ...
    try:
        ...
        if not hasattr(calculate_geomagnetic_coordinates, '_error_logged'):
            ...
            if "No module named 'apexpy'" in str(e):
                logger.info("Install apexpy with: pip install apexpy")
...
def add_geomagnetic_coordinates(coords_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if successful_calculations > 0:
        ...
        logger.warning(f"Geomagnetic coordinate calculation: {failed_calculations} failed (apexpy not installed)")
...
def main():
    ...
    try:
        ...
        logger.error("apexpy module not found. Please install it with: pip install apexpy")
        logger.info("Geomagnetic coordinate calculation will be skipped.")
    ...
    if len(coords_df) < min_stations:
        logger.error(f"CRITICAL: Insufficient stations ({len(coords_df)}) after catalogue build. Minimum required: {min_stations}")
        return False
    ...
    logger.info("Ready for coordinate validation (Step 1.2)")
    return True
