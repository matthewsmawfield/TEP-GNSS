#!/usr/bin/env python3
"""
TEP GNSS Analysis - STEP 3.6: TID Exclusion Analysis
=================================================
...
Requirements: Step 2.0 complete (Core TEP Correlation Analysis) and related geospatial data (Step 2.1)
Inputs:
  - data/processed/step_2_1_geospatial_{ac}.csv (from Step 2.1)
  - results/outputs/step_2_0_correlation_data_{ac}.csv (from Step 2.0)
  - results/outputs/step_4_3_wavelet-analysis_high_res_{ac}.json (from Step 4.3)
  - results/outputs/step_4_3_hilbert-if_high_res_{ac}.json (from Step 4.3)
Outputs:
  - results/outputs/step_3_6_tid_exclusion_comprehensive.json (comprehensive TID exclusion report)
  - results/figures/step_3_6_tid_exclusion_analysis.png (visualization plots)
...
def load_geospatial_data(analysis_center: str):
    ...
    try:
        ...
        data_file = ROOT / f"data/processed/step_2_1_geospatial_{analysis_center}.csv"
        if not data_file.exists():
            # Try results/outputs naming if present
            alt1 = ROOT / f"results/outputs/step_2_1_geospatial_data_{analysis_center}.csv"
            data_file = alt1 if alt1.exists() else data_file
        if not data_file.exists():
            # Fall back to step_3 correlation data (distance-binned summary)
            data_file = ROOT / f"results/outputs/step_2_0_correlation_data_{analysis_center}.csv"
            if not data_file.exists():
...
def analyze_temporal_band_separation(analysis_center: str = 'merged') -> Dict:
    ...
    try:
        ...
        wavelet_file = ROOT / f"results/outputs/step_4_3_wavelet-analysis_high_res_{analysis_center}.json"
        hilbert_file = ROOT / f"results/outputs/step_4_3_hilbert-if_high_res_{analysis_center}.json"
...
def analyze_trans_equatorial_propagation_exclusion(analysis_center: str) -> dict:
    ...
    try:
        ...
        try:
            geospatial_file = ROOT / "data" / "processed" / f"step_2_1_geospatial_{analysis_center}.csv"
...
def analyze_spatial_structure_comparison(analysis_center: str = 'merged') -> Dict:
    ...
    try:
        ...
        print_status("Comparing exponential correlation (TEP) vs plane-wave propagation (TID)", "INFO")
        ...
        try:
            ...
            if not data_file.exists():
                if analysis_center == 'merged':
                    ...
                    raise TEPFileError(f"Step 2.0 correlation data file not found for {analysis_center}")
        ...
        if 'dist_km' in df.columns and 'plateau_phase' in df.columns:
            ...
            return {'success': False, 'error': f'Step 2.1 geospatial data lacks coherence information needed for spatial structure analysis. Available columns: {available_columns}'}
...
def create_tid_exclusion_visualization(report_data: Dict, output_dir: Path):
    ...
    try:
        ...
        if not summary_data:
            ...
            plt.tight_layout()
            plot_path = output_dir / 'step_3_6_tid_exclusion_analysis.png'
...
def main():
    ...
    try:
        print_status("TEP GNSS Analysis Package v0.13 - STEP 3.6: TID Exclusion Analysis", "TITLE")
        ...
        report_path = output_dir / 'step_3_6_tid_exclusion_comprehensive.json'
