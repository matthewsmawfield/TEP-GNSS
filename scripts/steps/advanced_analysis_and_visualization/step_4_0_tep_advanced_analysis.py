        ...
        for bin_label, group in df.groupby('dist_bin', observed=True):
            ...
            try:
                ...
                if n > 1 and v_statistic >= 0: # V-test is directional, only meaningful if V_statistic is positive for target direction
                    ...
                    v_p = stats.norm.sf(Z) * 2 # Two-tailed p-value for general significance, or use one-tailed if direction specific
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Step 4.0 TEP Analysis - Advanced Validation")
    ...
    print_status("TEP GNSS Analysis Package v0.13 - STEP 4.0: Advanced Analysis", "TITLE")
    ...
    # Check prerequisites
    step_2_0_complete = (root_dir / 'logs/step_2_0_correlation_analysis.json').exists() # Updated from step_3_correlation_analysis.json
    if not step_2_0_complete:
        print_status("Step 2.0 must be completed before running Step 4.0", "ERROR") # Updated from Step 3 and Step 7
        return False
    ...
    print("\n" + "="*80)
    print("STEP 4.0 ADVANCED ANALYSIS COMPLETE")
    print("="*80)
    print_status(f"Results saved: {output_file}", "SUCCESS")
    print_status("Note: Anisotropy/azimuth analyses are performed in Step 2.2.", "INFO") # Updated from Step 5.
