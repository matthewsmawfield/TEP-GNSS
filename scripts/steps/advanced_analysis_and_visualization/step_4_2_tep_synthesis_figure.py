...
def create_panel_a(ax):
    ...
    for i, (center, color, label, marker) in enumerate(zip(centers, colors, labels, markers)):
        try:
            ...
            with open(results_dir / 'outputs' / f'step_2_0_correlation_{center}.json', 'r') as f:
                data = json.load(f)
...
def create_panel_b(ax):
    ...
    for center in ['code', 'igs_combined', 'esa_final']:
        try:
            with open(results_dir / 'outputs' / f'step_3_2_null_tests_{center}.json', 'r') as f:
                data = json.load(f)
...
def create_panel_c(ax):
    ...
    try:
        ...
        df = pd.read_csv(data_dir / 'processed' / 'step_2_1_geospatial_code.csv')
...
def main():
    ...
    output_path = ROOT / 'results' / 'figures' / 'step_4_2_tep_synthesis_figure.png'
