#!/usr/bin/env python3
"""
TEP GNSS Analysis - STEP 4.3: High-Resolution Astronomical Event Analysis
========================================================================
...
Requirements: Step 1.1 complete (Data Acquisition) and Step 1.2 complete (Coordinate Validation)
Inputs:
...
Outputs:
  - results/outputs/step_4_3_high_resolution_astronomical_events.json
Next: Step 4.4 (Gravitational Temporal Field Analysis)
...
import pywt
from scipy.optimize import curve_fit # Import curve_fit explicitly
import json
import warnings
...
step_logger = TEPLogger(
    name="step_4_3_high_resolution_astronomical_events",
    level="DEBUG",
    log_file_path=Path(__file__).resolve().parents[3] / "logs" / "step_4_3_high_resolution_astronomical_events.log"
)

# Register step logger so print_status uses it
set_step_logger(step_logger)
from scripts.utils.exceptions import (
    SafeErrorHandler, TEPDataError, TEPFileError, TEPAnalysisError,
    safe_json_read, safe_csv_read, safe_json_write, validate_file_exists, validate_directory_exists
)
...
def analyze_solar_eclipse_high_resolution(analysis_center: str = 'igs_combined') -> Dict:
    ...
    try:
        ...
        # Check if data is available for eclipse date
        data_dir = PACKAGE_ROOT / "data" / "raw" / analysis_center
        if not data_dir.exists():
            return {
                'error': f'No CLK files found for eclipse date {eclipse_date}',
        ...
        # Check if any eclipse files exist
        available_files = [f for f in eclipse_files if f.exists()]
        if not available_files:
            return {
                ...
                'eclipse_date': eclipse_date.isoformat(),
                'analysis_center': analysis_center,
                'searched_files': [str(f) for f in eclipse_files]
            }
...
def analyze_eclipse_differential_coherence(analysis_center: str = 'merged', resolution: str = '1min', eclipse_date: str = '2024-04-08') -> Dict:
    ...
    try:
        ...
        station_coords = pd.read_csv(PACKAGE_ROOT / "data/coordinates/step_1_1_station_coords_global.csv")
        ...
        for center in centers_to_load:
            ...
            data_dir = PACKAGE_ROOT / "data" / "raw" / center
            prefixes = {
                'code': 'COD0OPSFIN_',
                'igs_combined': 'IGS0OPSFIN_',
                'esa_final': 'ESA0OPSFIN_'
...
def plot_supermoon_perigee_curves() -> Optional[str]:
    ...
    try:
        ...
        site_dir = PACKAGE_ROOT / 'site' / 'figures'
        try:
...
def analyze_hilbert_instantaneous_frequency(analysis_center: str = 'merged') -> Dict:
    ...
    try:
        ...
        plot_path = PACKAGE_ROOT / 'results' / 'figures' / f'step_4_3_hilbert_if_{analysis_center}.png'
        plt.tight_layout()
...
def analyze_wavelet_time_frequency(analysis_center: str = 'merged', verbose: bool = True) -> Dict:
    ...
    try:
        ...
        plot_path = PACKAGE_ROOT / "results" / "figures" / plot_filename
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
...
def analyze_comprehensive_step_4_3(analysis_center: str = 'merged', resolution: str = '1min') -> Dict:
    """
    Run all Step 4.3 analyses: eclipses, planetary oppositions, wavelet, and Hilbert-IF.
    This is the default comprehensive analysis.
    """
    try:
        print_status("Starting COMPREHENSIVE Step 4.3 Analysis...", "PROCESS")
        ...
        comprehensive_results['interpretation'] = f"Comprehensive Step 4.3 analysis complete. {successful_analyses}/{total_analyses} analyses successful. Key findings: {'; '.join(key_findings)}"
...
def main():
    """
    Main function to run the high-resolution astronomical events analysis.
    """
    import argparse
    
    parser = argparse.ArgumentParser(description="TEP GNSS High-Resolution Astronomical Events - Step 4.3")
    ...
    print("=" * 80)
    print("TEP GNSS Analysis Package v0.16")
    ...
    if results.get('success'):
        ...
        # Save results
        output_dir = PACKAGE_ROOT / "results" / "outputs"
    ...
    print_status(f"STEP 4.3 COMPLETED in {elapsed_time:.1f} seconds", "TITLE")
    print_status("=" * 80, "INFO")
    print_status(f"Ready for Step 4.4 (Gravitational Temporal Field Analysis)", "INFO")
...
if __name__ == "__main__":
    try:
        ...
        print_status("Step 4.3 interrupted by user", "WARNING")
        sys.exit(1)
    except Exception as e:
        print_status(f"Step 4.3 failed - unexpected error: {e}", "ERROR")
        import traceback
        traceback.print_exc()
        sys.exit(1)
