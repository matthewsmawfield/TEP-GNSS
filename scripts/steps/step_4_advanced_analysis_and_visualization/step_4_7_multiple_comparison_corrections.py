#!/usr/bin/env python3
"""
TEP GNSS Analysis - Step 4.7: Multiple Comparison Corrections
===========================================================

Systematic application of formal multiple comparison corrections to all statistical tests
performed across Steps 2.0-4.6. Implements Bonferroni, False Discovery Rate (FDR), and 
Family-wise Error Rate (FWER) corrections to control Type I error inflation.

Requirements: All previous analysis steps (Steps 2.0-4.6) that generate statistical tests should be completed.
Inputs:
  - results/outputs/step_2_0_correlation_{ac}.json
  - results/outputs/step_2_2_geospatial_temporal_analysis_{ac}.json (and related sub-analyses)
  - results/outputs/step_3_0_cross_validation_suite_{ac}.json
  - results/outputs/step_3_1_robust_block_bootstrap_{ac}.json
  - results/outputs/step_3_2_null_tests_{ac}.json
  - results/outputs/step_3_3_validation_report.json (and related bias analysis outputs)
  - results/outputs/step_3_4_geographic_bias_validation.json
  - results/outputs/step_3_5_realistic_ionospheric_validation.json
  - results/outputs/step_3_6_tid_exclusion_comprehensive.json
  - results/outputs/step_4_0_advanced_analysis.json (and related sub-analyses)
  - results/outputs/step_4_3_high_resolution_astronomical_events.json (and related sub-analyses)
  - results/outputs/step_4_4_gravitational_temporal_field_analysis.json
  - results/outputs/step_4_5_comprehensive_diurnal_analysis.json
Outputs:
  - results/outputs/step_4_7_multiple_comparison_corrections.json (detailed corrections)
  - results/outputs/step_4_7_corrected_significance_summary.json (summary report)
  - results/outputs/step_4_7_correction_impact_analysis.csv (impact data)
  - results/figures/step_4_7_correction_impact_visualization.png
  - results/figures/step_4_7_significance_before_after.png
...
step_logger = TEPLogger(
    name="step_4_7_multiple_comparison_corrections",
    level="DEBUG",
    log_file_path=Path(__file__).resolve().parents[3] / "logs" / "step_4_7_multiple_comparison_corrections.log"
)
...
class MultipleComparisonCorrector:
    ...
    def __init__(self):
        ...
        self.test_registry = {
            ...
            'model_comparison': [],      # Step 2.0: Alternative model tests
            'cross_validation': [],      # Step 3.0: Cross-validation suite
            'null_validation': [],       # Step 3.2: Null hypothesis tests
            'advanced_analysis': [],     # Step 4.0: Advanced statistical analysis
            'astronomical_events': [],   # Step 4.3: Astronomical events and high-res analysis
            'gravitational_coupling': [],# Step 4.4: Planetary correlations
            'geographic_validation': [], # Step 3.4: Geographic bias tests
            'ionospheric_validation': [],# Step 3.5: Ionospheric independence
            'diurnal_analysis': []       # Step 4.5: Temporal pattern analysis
        }
    ...
    def collect_statistical_tests(self) -> Dict:
        ...
        self._collect_step3_0_tests(results_dir)
        ...
        # Step 4.3: Astronomical Events
        self._collect_step4_3_tests(results_dir)
        
        # Step 4.4: Gravitational Coupling
        self._collect_step4_4_tests(results_dir)
        
        # Step 3.4: Geographic Validation
        self._collect_step3_4_tests(results_dir)
        
        # Step 3.5: Ionospheric Validation
        self._collect_step3_5_tests(results_dir)
        
        # Step 4.5: Diurnal Analysis
        self._collect_step4_5_tests(results_dir)
    ...
    def _collect_step2_0_tests(self, results_dir: Path):
        """Collect p-values from Step 2.0 correlation analysis"""
                ...
                try:
                    ...
                    if 'best_fit' in data and 'r_squared' in data['best_fit']:
                        ...
                        if n_bins > 3:
                            ...
                            print_status(f"WARNING: Insufficient bins ({n_bins}) for reliable F-test in Step 2.0 exponential fit for {ac.upper()}. Skipping p-value calculation.", "WARNING")
                    
                    # Model comparison tests - now collected from Step 4.0
                    # Step 2.0 doesn't have p-values, Step 4.0 does the rigorous comparison
                    pass  # Model comparison p-values collected in _collect_step4_0_tests
        ...
        for ac in ['code', 'esa_final', 'igs_combined']:
            ...
            if step3_0_file.exists():
                try:
                    ...
                    # LOSO analysis - now has bootstrap CV significance test
                    if 'loso_analysis' in data and data['loso_analysis'].get('success'):
                        if 'lambda_stability' in data['loso_analysis'] and 'cv_p_value' in data['loso_analysis']['lambda_stability']:
                            stability = data['loso_analysis']['lambda_stability']
                            self.test_registry['cross_validation'].append({
                                'test_name': f'loso_stability_{ac}',
                                'p_value': float(stability['cv_p_value']),
                                'test_statistic': stability['cv_z_score'],
                                'description': f'LOSO lambda stability (CV={stability["cv_lambda"]:.3f}) for {ac.upper()}'
                            })
                        else:
                            cv = data['loso_analysis'].get('coefficient_of_variation', 0)
                            print_status(f"WARNING: LOSO analysis missing cv_p_value for {ac.upper()}. Old output format? CV={cv:.3f}", "WARNING")
                    
                    # LODO analysis - now has bootstrap CV significance test
                    if 'lodo_analysis' in data and data['lodo_analysis'].get('success'):
                        if 'lambda_stability' in data['lodo_analysis'] and 'cv_p_value' in data['lodo_analysis']['lambda_stability']:
                            stability = data['lodo_analysis']['lambda_stability']
                            self.test_registry['cross_validation'].append({
                                'test_name': f'lodo_stability_{ac}',
                                'p_value': float(stability['cv_p_value']),
                                'test_statistic': stability['cv_z_score'],
                                'description': f'LODO lambda stability (CV={stability["cv_lambda"]:.3f}) for {ac.upper()}'
                            })
                        else:
                            cv = data['lodo_analysis'].get('coefficient_of_variation', 0)
                            print_status(f"WARNING: LODO analysis missing cv_p_value for {ac.upper()}. Old output format? CV={cv:.3f}", "WARNING")
    ...
    def _collect_step4_0_tests(self, results_dir: Path):
        ...
        if step4_0_file.exists():
            try:
                ...
                # Model comparison tests (rigorous likelihood ratio tests)
                if 'results' in data and 'model_comparison' in data['results']:
                    for ac, ac_models in data['results']['model_comparison'].items():
                        for model_name, model_data in ac_models.items():
                            if model_name != 'Exponential' and 'lr_test_vs_exponential' in model_data:
                                lr_test = model_data['lr_test_vs_exponential']
                                self.test_registry['model_comparison'].append({
                                    'test_name': f'model_{model_name}_{ac}',
                                    'p_value': float(lr_test['p_value']),
                                    'test_statistic': lr_test['lr_statistic'],
                                    'description': f'{model_name} vs Exponential for {ac.upper()}'
                                })
                            ...
                            for quintile, q_data in ac_data['quintile_analysis'].items():
                                if 'r_squared' in q_data and 'n_bins' in q_data:
                                    ...
                                    if n_bins > 3:
                                        ...
                                        print_status(f"WARNING: Insufficient bins ({n_bins}) for reliable F-test in Step 4.0 elevation dependence ({quintile}) for {ac.upper()}. Skipping p-value calculation.", "WARNING")
    ...
    def _collect_step4_3_tests(self, results_dir: Path):
        """Collect p-values from Step 4.3 astronomical events"""
        step4_3_files = [
            'step_4_3_comprehensive_high_res_all-centers.json',
            'step_4_3_supermoon_high_res_all-centers.json',
            'step_4_3_comprehensive_eclipses_all-centers.json'
        ]
        
        for filename in step4_3_files:
            step4_3_file = results_dir / filename
            if step4_3_file.exists():
                try:
                    with open(step4_3_file, 'r') as f:
                        data = json.load(f)
    ...
    def _collect_step4_4_tests(self, results_dir: Path):
        """Collect p-values from Step 4.4 gravitational coupling - FIXED VERSION"""
        step4_4_file = results_dir / 'step_4_4_comprehensive_analysis_results.json'
        if step4_4_file.exists():
            try:
                ...
                print_status(f"Step 4.4: Collected {len([t for t in self.test_registry['gravitational_coupling']])} gravitational coupling tests", "INFO")
            
            except Exception as e:
                print_status(f"ERROR: Could not parse Step 4.4 results: {e}", "WARNING")
                import traceback
                traceback.print_exc()
    
    def _collect_step3_4_tests(self, results_dir: Path):
        """Collect p-values from Step 3.4 geographic validation"""
                ...
                if 'validation_results' in data and 'geographic_consistency' in data['validation_results']:
                    for ac, ac_data in data['validation_results']['geographic_consistency'].items():
                        if 'lambda_std' in ac_data and 'lambda_mean' in ac_data:
                            ...
                            # Removed heuristic p-value calculation (p_value = min(1.0, cv * 10)).
                            # Coefficient of Variation is a measure of variability, not a p-value for hypothesis testing.
                            # If a statistical test for the significance of CV is available, it should be implemented here.
                            print_status(f"WARNING: No statistically sound p-value for geographic consistency (CV={cv:.3f}) for {ac.upper()}. Skipping this test.", "WARNING")
                            # self.test_registry['geographic_validation'].append({
                            #     'test_name': f'geographic_consistency_{ac}',
                            #     'p_value': float(cv), # Store the CV value directly
                            #     'test_statistic': cv,
    ...
    def _collect_step3_5_tests(self, results_dir: Path):
        """Collect p-values from Step 3.5 ionospheric validation"""
        step3_5_file = results_dir / 'step_3_5_realistic_ionospheric_validation.json'
        if step3_5_file.exists():
            try:
                with open(step3_5_file, 'r') as f:
                    data = json.load(f)
                ...
                print_status(f"Warning: Could not parse Step 3.5 results: {e}", "WARNING")
    
    def _collect_step4_5_tests(self, results_dir: Path):
        """Collect p-values from Step 4.5 diurnal analysis"""
        step4_5_file = results_dir / 'step_4_5_comprehensive_diurnal_analysis.json'
        if step4_5_file.exists():
            try:
                ...
                for ac in ['code', 'esa_final', 'igs_combined']:
                    if ac in data:
                        ...
                        if 'day_night_ratio' in ac_data:
                            ...
                            # Removed heuristic p-value calculation (p_value = max(0.001, min(1.0, 1.0 - deviation * 10))).
                            # If a statistical test for the significance of the deviation is available, it should be implemented here.
                            print_status(f"WARNING: No statistically sound p-value for diurnal modulation (deviation={deviation:.3f}) for {ac.upper()}. Skipping this test.", "WARNING")
                            # self.test_registry['diurnal_analysis'].append({
                            #     'test_name': f'diurnal_modulation_{ac}',
                            #     'p_value': float(deviation), # Store the deviation value directly
                        ...
                        if 'coherence_cv' in ac_data:
                            ...
                            # Removed heuristic p-value calculation (p_value = min(1.0, cv / 10.0)).
                            # Coherence CV is a measure of variability, not a p-value for hypothesis testing.
                            # If a statistical test for the significance of CV is available, it should be implemented here.
                            print_status(f"WARNING: No statistically sound p-value for diurnal stability (CV={cv:.3f}) for {ac.upper()}. Skipping this test.", "WARNING")
                            # self.test_registry['diurnal_analysis'].append({
                ...
                print_status(f"Warning: Could not parse Step 4.5 results: {e}", "WARNING")
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if key == 'permutation_p_value' and isinstance(value, (int, float)):
                    ...
                    # Removed heuristic p-value assignment. If statistical significance is directly
                    # reported, the original p-value from that test should be used. Otherwise, raise an error.
                    raise TEPAnalysisError(f"Heuristic p-value for 'statistically_significant' encountered in {source_name}. Original p-value is required. Aborting to avoid 'mock' values.")
    ...
    def apply_fdr_correction(self, all_tests: List[Dict], method: str = 'bh') -> Dict:
        ...
        try:
            ...
            print_status(f"ERROR: FDR correction failed: {e}. Raising TEPAnalysisError as per user instructions to avoid fallbacks.", "ERROR")
            raise TEPAnalysisError(f"FDR correction failed: {e}")
    ...
    def create_visualizations(self, corrections: Dict, summary: Dict):
        ...
        plt.savefig(figures_dir / 'step_4_7_correction_impact_visualization.png', 
        ...
        plt.tight_layout()
        plt.savefig(figures_dir / 'step_4_7_significance_before_after.png', 
                   dpi=300, bbox_inches='tight')
...
def main():
    ...
    print_status(f"TEP GNSS Analysis Package {VERSION_STRING} - STEP 4.7: Multiple Comparison Corrections", "TITLE")
    ...
    try:
        ...
        with open(outputs_dir / 'step_4_7_multiple_comparison_corrections.json', 'w') as f:
            json.dump(corrections, f, indent=2)
        
        # Save summary
        with open(outputs_dir / 'step_4_7_corrected_significance_summary.json', 'w') as f:
        ...
        df.to_csv(outputs_dir / 'step_4_7_correction_impact_analysis.csv', index=False)
        ...
        print_status(f"Multiple comparison corrections failed: {e}", "ERROR")
        import traceback
        traceback.print_exc()
