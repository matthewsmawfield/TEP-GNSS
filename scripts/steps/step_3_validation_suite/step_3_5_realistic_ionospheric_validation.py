#!/usr/bin/env python3
"""
TEP GNSS Analysis - STEP 3.5: Realistic Ionospheric Validation with Available Real Data
======================================================================================
...
Inputs:
  ...
  - data/raw/{igs,esa,code}/*.CLK.gz files (from Step 1.1, for data parsing)
Outputs:
...
from scripts.utils.exceptions import TEPAnalysisError, TEPFileError, TEPDataError
...
def extract_real_tep_coherence_time_series() -> pd.DataFrame:
    ...
    # Assign a fixed date for aggregated results
    fixed_date = datetime(2023, 1, 1) # Arbitrary fixed date for aggregated data

    for center in centers:
        print_status(f"Processing {center.upper()} aggregated correlation results...", "PROCESS")

        # Construct the file path for the aggregated JSON output from Step 2.0
        file_path = PACKAGE_ROOT / f"results/outputs/step_2_0_correlation_{center}.json"

        if not file_path.exists():
            print_status(f"WARNING: Aggregated correlation file not found for {center}: {file_path}. Skipping.", "WARNING")
            continue

        try:
            # Load JSON correlation results
            with open(file_path, 'r') as f:
                correlation_data = json.load(f)

            # Extract r_squared from exponential_fit as a proxy for coherence
            # Check if exponential_fit exists and has r_squared
            if 'exponential_fit' in correlation_data and correlation_data['exponential_fit'] and 'r_squared' in correlation_data['exponential_fit']:
                ...
                if r_squared_value is not None:
                    if fixed_date not in all_daily_data:
                        all_daily_data[fixed_date] = []
                    all_daily_data[fixed_date].append(r_squared_value) # Use r_squared as the coherence value
                    print_status(f"Successfully extracted r_squared from {file_path} for {center.upper()}", "INFO")
                else:
    ...
    if not daily_aggregated:
        raise TEPDataError("No real TEP coherence data could be extracted from any analysis center.")
    ...
    for year, month in sample_months:
        ...
        try:
            ...
            if monthly_kp:
                ...
                print_status(f"WARNING: No valid Kp data parsed from {filename}. Skipping.", "WARNING")
            
        except Exception as e:
            print_status(f"ERROR: Failed to download or parse {filename}: {e}. Skipping.", "ERROR")
            continue
    
    if not all_kp_data:
        raise TEPFileError("No real Kp data could be downloaded or parsed from GFZ Potsdam for the specified period.")
    ...
    try:
        ...
        for entry in f107_json:
            try:
                ...
                print_status(f"WARNING: Error parsing F10.7 entry ({entry.get('time-tag','N/A')}): {e}. Skipping entry.", "WARNING")
                continue
        
        if not f107_data:
            raise TEPDataError(f"No F10.7 data available in the NOAA dataset for the analysis period: {start_date} to {end_date}.")
        ...
        raise TEPFileError(f"Failed to download or process real F10.7 data from NOAA: {e}")
...
def analyze_real_data_correlations(tep_coherence: pd.DataFrame, kp_data: pd.DataFrame, f107_data: pd.DataFrame) -> Dict:
    ...
    # Comprehensive diurnal analysis provided by Step 4.5 with hourly windowing methodology
...
def generate_realistic_validation_assessment(analysis_results: Dict) -> Dict:
    ...
    # Comprehensive diurnal analysis available through Step 4.5
    ...
    realistic_assessment = {
        ...
        'data_limitations': {
            ...
            'diurnal_analysis': 'Insufficient temporal resolution in daily-aggregated data (Step 4.5 provides comprehensive diurnal analysis)'
        },
...
def main():
    ...
    try:
        ...
        print_status("⚠️  Step 3.5: Insufficient temporal resolution for diurnal analysis", "WARNING")
        print_status("✅ Step 4.5: Comprehensive diurnal analysis with hourly windowing", "SUCCESS")
        ...
        raise TEPAnalysisError(f"Realistic ionospheric validation failed: {e}") # Re-raise a specific exception
