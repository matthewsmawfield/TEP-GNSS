...
step_logger = TEPLogger(
    name="step_2_0_correlation_analysis",
    level="DEBUG",
    log_file_path=ROOT / "logs" / "step_2_0_correlation_analysis.log"
)
...
def process_file_worker(clk_file: Path):
    ...
    try:
        ...
        write_pair_level = TEPConfig.get_bool('TEP_WRITE_PAIR_LEVEL', True)
        ...
        if write_pair_level and pair_data_buffer:
            # Create output directory for pair files
            pair_output_dir = ROOT / "results/tmp"
...
def process_analysis_center(ac: str, coords_df, max_files: int = None, distance_cache: Dict[Tuple[str, str], float] = None):
    ...
    write_pair_level = TEPConfig.get_bool('TEP_WRITE_PAIR_LEVEL', True)
    max_memory_pairs = int(os.getenv('TEP_MAX_MEMORY_PAIRS', '500000'))  # 500K pairs max in memory
...
def main():
    ...
    # Save global summary
    global_summary_path = PACKAGE_ROOT / "results" / "outputs" / "step_2_0_correlation_analysis_summary.json"
    ...
    try:
        provenance_success = update_provenance_snapshot('step_2_0_tep_correlation_analysis', results)
