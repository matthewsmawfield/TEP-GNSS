...
def build_metadata(cff_data):
    ...
    # ModifyDate should reflect the last modification date, not the original release date
    # Source from VERSION.json if available, otherwise fall back to date-released
    modify_date = ''
    try:
        ...
        if version_path.exists():
            ...
            modify_date = version_data.get('date', '')
    except Exception:
        pass
    if not modify_date:
        modify_date = date
    if modify_date:
        modify_pdf = str(modify_date).replace('-', ':')
        metadata['ModifyDate'] = f'{modify_pdf} 00:00:00'
