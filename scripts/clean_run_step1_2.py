...
Usage:
    python scripts/clean_run_step1_2.py [--dry-run] [--skip-cleanup] [--start-step STEP]

Options:
    --dry-run       Show what would be cleaned without actually cleaning
    --skip-cleanup  Skip cleanup and only run the steps
    --start-step    Start from specific step (e.g., "1.1", "2.0"). Default: "1.0"

Examples:
    python scripts/clean_run_step1_2.py                    # Run everything from Step 1.0
    python scripts/clean_run_step1_2.py --start-step 2.0   # Run from Step 2.0 onwards
    python scripts/clean_run_step1_2.py --start-step 1.1   # Run from Step 1.1 onwards
    python scripts/clean_run_step1_2.py --dry-run          # See what would be cleaned
    python scripts/clean_run_step1_2.py --skip-cleanup     # Skip cleanup phase
...
def get_cleanup_targets() -> Dict[str, List[Path]]:
    ...
    targets = {
        ...
        # Output files (Step 1 & 2) - Clean entire outputs directory
        'output_files': [
...
def execute_step1_2_pipeline(start_step: str = '1.0') -> bool:
    """
    Execute the complete Step 1 & 2 pipeline sequentially.
    
    Args:
        start_step: Step to start from (e.g., '1.1', '2.0')
    ...
    if start_index == 0 and start_step != '1.0':
        logger.error(f"Invalid start step: {start_step}. Available steps: 1.0, 1.1, 1.2, 2.0, 2.1, 2.2")
        return False
    
    filtered_steps = steps[start_index:]
    ...
    for i, step in enumerate(filtered_steps, 1):
        logger.info(f"Starting step {i}/{len(filtered_steps)}: {step['name']}")
    ...
    if successful_steps == len(filtered_steps):
        ...
        logger.error(f"Pipeline failed: {successful_steps}/{len(filtered_steps)} steps completed")
        return False
...
def validate_clean_run() -> bool:
    ...
    expected_outputs = [
        ...
        PROJECT_ROOT / "logs" / "step_2_2_geospatial_temporal_analysis_code.json",
...
def main():
    ...
    parser.add_argument('--start-step', type=str, default='1.0',
                       help='Start from specific step (e.g., "1.1", "2.0"). Default: "1.0"')
    ...
    if not args.dry_run:
        pipeline_success = execute_step1_2_pipeline(args.start_step)
